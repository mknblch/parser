...
public class TmpLexer {
    ...
    private int offset;
    public TmpLexer(Grammar grammar) {
        this.grammar = grammar;
    }

    public void setInput (final CharSequence input) throws SyntaxException {
        this.input = input;
        reset();
    }
    ...
    public void arm(Map<String, Pattern> rules) {
        this.rules = rules;
    }

    public Token nextToken() throws SyntaxException {

        while(consumeExcludes());
    ...
    private boolean consumeExcludes() {
        final CharSequence rest = input.subSequence(offset, input.length());
